...
if __name__ == "__main__":
    ...
    logger.info(f"🖥️⚙️ {Colors.apply('[PARAM]').blue} STT backend: {Colors.apply(STT_BACKEND).blue}")
...
async def lifespan(app: FastAPI):
    ...
    Initializes global components like SpeechPipelineManager and AudioInputProcessor
    and stores them in `app.state`. Handles cleanup on shutdown.

    Args:
        app: The FastAPI application instance.
    """
    logger.info("🖥️▶️ Server starting up")
    # Ensure required API keys are present when using API-based engines/backends
    if TTS_START_ENGINE == "deepgram" or STT_BACKEND == "deepgram":
        if not os.getenv("DEEPGRAM_API_KEY"):
            raise RuntimeError("DEEPGRAM_API_KEY is not set. Add it to .enve/.env before starting the server.")
    if LLM_START_PROVIDER == "megallm" and not os.getenv("MEGALLM_API_KEY"):
        raise RuntimeError("MEGALLM_API_KEY is not set. Add it to .enve/.env before starting the server.")
...
async def process_incoming_data(ws: WebSocket, app: FastAPI, incoming_chunks: asyncio.Queue, callbacks: 'TranscriptionCallbacks') -> None:
    ...
    (e.g., updates client TTS state via `callbacks`, clears history).
    ...
    try:
        ...
        logger.warning(f"🖥️⚠️ Runtime error in process_incoming_data (likely closed connection): {repr(e)}")
...
async def send_tts_chunks(app: FastAPI, message_queue: asyncio.Queue, callbacks: 'TranscriptionCallbacks') -> None:
    ...
    queue, base64 encodes them, and puts them onto the outgoing `message_queue`
        ...
        while True:
            ...
            try:
                ...
                if not final_expected or audio_final_finished:
                    ...
                    callbacks.tts_client_playing = False
                    # Clear mic interruption when generation is fully done
                    app.state.AudioInputProcessor.interrupted = False
                    callbacks.interruption_time = 0
            ...
            # Direct Base64 encoding (API-based TTS sends high-quality audio, no upsampling needed)
...
class TranscriptionCallbacks:
    ...
    def on_before_final(self, audio: bytes, txt: str):
        ...
        # Defer sending the final user request until on_final so we send the true final text
    ...
    def on_final(self, txt: str):
        ...
        # Send the final user request to the client (full text)
    ...
    def on_recording_start(self):
        ...
        # If TTS is currently playing, do NOT abort the generation; just pause mic intake
        if self.tts_client_playing:
            logger.info(f"{Colors.apply('🖥️ℹ️ Ignoring recording start while TTS is playing').blue}")
            # Temporarily mark mic interrupted to reduce STT re-entries during playback
            self.app.state.AudioInputProcessor.interrupted = True
