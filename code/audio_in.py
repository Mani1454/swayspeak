...
from transcribe import create_transcription_processor
...
class AudioInputProcessor:
    ...
    def __init__(
        ...
        # ✅ DYNAMIC: Reads "STT_BACKEND" from .enve (defaults to deepgram if missing)
        stt_backend = os.getenv("STT_BACKEND", "deepgram").lower()
        self.transcriber = create_transcription_processor(
            source_language=language,
            backend=stt_backend,  # <--- Uses variable, not hardcoded string
    ...
    async def _run_transcription_loop(self) -> None:
        ...
        while True: # Loop restored to continuously call transcribe_loop
            try:
                if getattr(self.transcriber, "shutdown_performed", False):
                    logger.info(f"👂⏹️ Transcriber reports shutdown; exiting {task_name}.")
                    break
