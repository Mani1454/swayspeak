...
class SpeechPipelineManager:
    ...
    def __init__(
            self,
            tts_engine: str = None,   # Set defaults to None so we can check env
            llm_provider: str = None,
            llm_model: str = None,
        ):
        """
        Initializes the SpeechPipelineManager with .enve variables.
        ...
        Args:
            tts_engine: The TTS engine to use (e.g., "deepgram", "kokoro"). If None, reads from TTS_ENGINE env var.
            llm_provider: The LLM backend provider (e.g., "groq", "openai"). If None, reads from LLM_PROVIDER env var.
            llm_model: The specific LLM model identifier. If None, reads from LLM_MODEL env var.
        """
        # ✅ DYNAMIC: Load from .enve, fallback to "deepgram"/"groq" only if .enve is empty
        self.tts_engine = tts_engine if tts_engine else os.getenv("TTS_ENGINE", "deepgram")
        self.llm_provider = llm_provider if llm_provider else os.getenv("LLM_PROVIDER", "groq")
        self.llm_model = llm_model if llm_model else os.getenv("LLM_MODEL", "llama3-8b-8192")
        ...
        # API latency is variable, no need to measure
    ...
    def _llm_inference_worker(self):
        ...
        text, checks for a natural sentence boundary to define the `quick_answer` using
        `TextContext`. If a quick answer is found, it signals `llm_answer_ready_event`.
        Handles stop requests (`stop_llm_request_event`) and signals completion/abortion
        via `stop_llm_finished_event` and internal flags. Runs until `shutdown_event` is set.
        """
